...
class Agent:
    ...
    def init_nn(self):
        self.state_layer = tf.placeholder(tf.float32, [None, self.lane_len*12 + 4], 'state')
        self.lane_layers = [tf.slice(self.state_layer, [0, i*self.lane_len], [-1, self.lane_len]) for i in range(0, 12)]
        self.phase = tf.slice(self.state_layer, [0, 12*self.lane_len], [-1, 4])
        self.subsize = 4
        ...
        with tf.name_scope('subnet'):
            ...
            self.sub_layers = []
            self.sub_layers.append(layer[0]+layer[4]+layer[8])
            self.sub_layers.append(layer[2]+layer[6]+layer[9])
            self.sub_layers.append(layer[1]+layer[5]+layer[10])
            self.sub_layers.append(layer[3]+layer[7]+layer[11])
